...
During preparation for class I sometimes think up of animation that will explain the concept I am teaching. To share the resulting animations with a wider audience I often create a gif an post it on twitter vie [@rafalab](). After seeing one of these John Storey [asked](https://twitter.com/johnstorey/status/892032669021818881) if the source code for theses gifs is publicly available. They are not. The reason is that I am not that organized and these come about during last minute preparations. But since Storey asked, I have organized the code and include it below. Remember these were done last minute so please don't judge me. Actually you can critique all you want, that's how I learn. 

The basic way all these figs here are made is that I make several png files then convert them to an animated gif that simply pages through these. These tools I have used are `saveGIF` from the animation package, the `gganimate` from the `gganimate` package and the online [Animated GIF maker](https://ezgif.com/maker). The last one I used when only a couple of images are involved and I want phasing effect. 
...
```{r, message=FALSE, comment=FALSE, fig.width=8, fig.height=3}
library(tidyverse)
library(gridExtra)
library(dslabs)
ds_theme_set()
...
names(dat) <- c("X", "Y", "Z")
p1 <- dat %>% ggplot(aes(X,Y)) + geom_point(alpha = .5) +
...
p2 <- dat %>% ggplot(aes(X, Y, color = Z)) + 
  geom_point(show.legend = FALSE, alpha = 0.5) +
  ggtitle(paste("correlations =",  paste(signif(corrs,2), collapse=" "))) +  
  theme(plot.title = element_text(size = 8, face = "bold"))

p3 <- p2 + annotate("text", x = means$x, y = means$y, label = paste("Z=", means$z), cex = 5)  
...
p1 <- surv_income %>% ggplot(aes(income, infant_survival_rate, label = group, color = group)) +
...
The first animation I shared explains how local regression works. Basically, for each point assign positive weights to points close that point, fit a line with weighted regression, keep the fitted value for the point in question, move to the next point. Here is the gif:

```{r, echo=FALSE, fig.align='center'}
knitr::include_graphics("https://raw.githubusercontent.com/simplystats/simplystats.github.io/master/_images/loess.gif")
```

The data here comes from a microarray experiment. I used the `animation` package to save the gif. 

```{r, warning=FALSE, comment=FALSE, eval=FALSE}
##rafalib::install_bioc("SpikeIn")
rafalib::mypar()
library(SpikeIn)
data(SpikeIn95)
spms <- pm(SpikeIn95)
...
This animation from from Hans Rosling's talk [New Insights on Poverty](https://www.ted.com/talks/hans_rosling_reveals_new_insights_on_poverty?language=en). We us
...
while(count3<=(N*M/2) & count2<=(N*M/2)){
  ...
  i<-i+1
}},'bayes.gif', interval = .1, ani.width = 800, ani.height = 500)
```


## Pacman

Finally, this is not really educational. I just wanted to show that Piecharts can be useful. 
...
Here is the code. Be careful when you run it as it will erase all your 
```{r, eval=FALSE}
saveGIF({
  N=10
  for(i in 0:(N-1)){
...
Note the R code fits in a [tweet](https://twitter.com/rafalab/status/631243286464704512).
