...
![User-developer spectrum](assets/userdeveloper.png)
...
or, to produce similar output as you might get with `dplyr`,
...
As time goes on, R will be inundated with a new population of users who take different things for granted (e.g. the tidyverse) and will have very different requirements. Thankfully, we can be confident that the R language is flexible enough to meet those requirements as long as the community is up for the challenge. 
